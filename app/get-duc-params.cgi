...
if [ -f "$CONFIG_FILE" ]; then
    ONE_FILE_SYSTEM=$(grep "^ONE_FILE_SYSTEM=" "$CONFIG_FILE" 2>/dev/null | cut -d'=' -f2)
...
ONE_FILE_SYSTEM=${ONE_FILE_SYSTEM:-"yes"}
...
echo "$(date): DUC parameters retrieved: one-file-system=$ONE_FILE_SYSTEM, check-hard-links=$CHECK_HARD_LINKS, max-depth=$MAX_DEPTH" >> "$LOG_FILE"
